...
import {AiOutlineSearch, AiOutlinePlus, AiOutlinePoweroff} from 'react-icons/ai'
...
const Event = () => {
    ...
    const handleDetailEvent = () => setOpenDetailEvent(!openDetailEvent)
    const [popUpEdit, setPopUpEdit] = useState(false)
    const handlePopUpEdit = () => setPopUpEdit(!popUpEdit)
    const [popUpEvent, setPopUpEvent] = useState(false)
    const handlePopUpEvent = () => setPopUpEvent(!popUpEvent)
  return (
    <div className='flex'>
        <div>
            <Sidebar/>
        ...
        <div className='w-full overflow-hidden'>
            ...
            <div className='mt-8 ml-5'>
                <p className='font-poppins font-semibold text-22 mb-1 tracking-tighter text-slate-700'>Event</p>
            </div>
            <div className='mx-5 -mt-1 mb-2 cursor-pointer flex justify-center p-1 rounded-full'>
                <AiOutlinePlus className='bg-create text-white text-25 rounded-full  hover:-translate-y-1 duration-200' onClick={handlePopUpEvent}/>
            </div>
            <div className='mt-3 mx-5 flex flex-wrap pb-7'>
                <div className='mr-3 mb-3 w-48 rounded-xl bg-event shadow-md shadow-blue-500/50 duration-300 hover:-translate-y-1'>
                    ...
                    <div className='mt-1 -mb-1 flex justify-evenly w-36 cursor-pointer mx-auto'>
                        <TbListDetails className='text-18 mt-1 text-white duration-200 hover:-translate-y-1 hover:text-green-300' onClick={handleDetailEvent}/>
        ...
        <div>
            {openDetailEvent && (<DetailEvent/>)}
        </div>
        <div>
            {popUpEdit && (<EditEvent/>)}
        </div>
        <div>
            {popUpEvent && (<CreateEvent/>)}
        </div>
    </div>
  )
}
